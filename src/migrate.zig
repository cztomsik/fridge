...
fn migrateObjects(db: *Session, pristine: *Session, kind: []const u8) !void {
    for (try sqlite_master.objects(pristine, kind)) |obj| {
        ...
        const curr = try db.query(sqlite_master).where(.type, kind).where(.name, obj.name).findFirst() orelse {
    ...
    for (try sqlite_master.objects(db, kind)) |obj| {
        if (try pristine.query(sqlite_master).where(.type, kind).where(.name, obj.name).findFirst() == null) {
            logObject(obj, .drop);
